...
int main(int argc, char **argv)
{
    init_sig_handle();
    if (argc != 2) {
        fprintf(stderr, "Usage : %s <TUNNEL_NAME>\n", argv[0]);
        exit(errno);
    }
    const char *const TUNNEL_NAME = argv[1];
    // Check the TUNNEL is exist
    if (access(TUNNEL_NAME, F_OK)) {
        fprintf(stderr, "The tunnel: %s is not exist!", TUNNEL_NAME);
        exit(errno);
    }

    EE(my_connect(TUNNEL_NAME));
    while (fputs("What word do you want : ", stderr)) {
        ...
        (void) lookup(&try_it, TUNNEL_NAME);
