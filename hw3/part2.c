...
int GetProcessesDirect(void)
{
    ...
    P = (struct Proc *) calloc(globbuf.gl_pathc, sizeof(struct Proc));
...
int GetProcesses(void)
{
    ...
    while (!!(fgets(line, MAXLINE, tn))) {
        ...
        int num = sscanf(line, PSFORMAT, PSVARS);
...
int GetRootPid(void)
{
    ...
    /* Still nothing. Maybe it is a container's entrypoint. We'll take
...
{
    int me;
    for (me = NProc - 1; me >= 0 && P[me].pid != pid; me--)
        ; /* Search process */
    return me;
}
...
/*
 * Rersion 2.33-scc 2021-10-23
 * Remove unused parts. Fork from https://github.com/tmm1/pstree.
 *
 * $Log: pstree.c,v $
